...
class PartnerDatatable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'backend.partner.action')
            ->addIndexColumn()
            ->editColumn('image', function($data) {
                return !is_null($data->image)
                    ? '<img src="' . asset('upload/partner/' . $data->image) . '" style="width:40px;height:40px;border-radius:50%;" />'
                    : '<img src="' . asset('backend/default.png') . '" style="width:40px;height:40px;border-radius:50%;" />';
            })
            ->editColumn('name', function($data) {
                return $data->name ?? 'لا يوجد اسم';
            })
