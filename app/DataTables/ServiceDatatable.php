...
class ServiceDatatable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'backend.service.action')
            ->addIndexColumn()
            ->editColumn('image', function($data) {
                return !is_null($data->image)
                    ? '<img src="' . asset('upload/service/' . $data->image) . '" style="width:40px;height:40px;border-radius:50%;" />'
            ...
            ->editColumn('slug', function($data) {
                return $data->getTranslation('slug', 'ar');
            })
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('slug', 'slug')->title('الرابط')->responsivePriority(0)->addClass('text-center'),
