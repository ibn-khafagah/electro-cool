...
class Feedbackdatatable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'backend.feedback.action')
            ->addIndexColumn()
            ->editColumn('image', function($data) {
                return !is_null($data->image)
                    ? '<img src="' . asset('upload/feedback/' . $data->image) . '" style="width:40px;height:40px;border-radius:50%;" />'
            ...
            ->editColumn('job', function($data) {
                return $data->getTranslation('job', 'ar');
            })
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('job', 'job')->title('المسمى الوظيفة')->responsivePriority(0)->addClass('text-center'),
